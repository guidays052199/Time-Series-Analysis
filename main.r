...
ts.plot(log_ret_brent, fitted(brent_model), col=c("red", "black"), ylab = "Real x Modelo Brent")
...
par(mfrow = c(2,1))
ts.plot(log_ret_petr, fitted(petr_model), col=c("red", "black"), ylab = "Real x Modelo Brent")
ts.plot(residuals(petr_model), col="blue", ylab="resíduo")

### Petrobrás x Brent Oil Prices ### 

par(mfrow=c(2,1))
model_arimax <- Arima(log_ret_petr, order = c(0,1,1), xreg =log_ret_brent, include.constant = FALSE)
...
forecast_barclays <- c(log(85) -  4.586599)  ### barclays forecast
...
### Ano de Eleições ### 
elections <- Arima(log_ret_petr, order = c(0,1,2), xreg = log_ret_brent)
